...
def gen_player_stats_sql_template(conn, fk_game_id):
    ...
    #Iván Marcano,Pepe Laveran Lima Ferreira,Zaidu Sanusi,Pepê Gabriel Aquino Cossa,Stephen Eustaquio,Mateus Uribe,Wenderson Galeno,Otávio Edmilson da Silva Monteiro,Mehdi Taremi,Francisco Evanilson,Toni Martínez,Fábio Cardoso,Carmo David,João Mário,Marko Grujić,Nico González,Gabriel Veron,Gonçalo Borges,Daniel Namaso Loader,Wendell Nascimento Borges,Wilson Manafá,Rodrigo Conceição,André Franco,Vasco Sousa,Bernardo Folha,Bruno Costa
    ...
    player_stats_cols = pd.read_sql("select column_name from information_schema.columns where table_name = 'player_stats' and column_name != 'player_stat_id'", conn).column_name.tolist()
