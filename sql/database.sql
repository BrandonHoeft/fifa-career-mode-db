...
create table if not exists player_gm_summary (

    pgs_id int primary key generated by default as identity,
...
comment on table player_gm_summary is 'each record represents a players overall stats (rating, minutes, running) in a single league game';

create table if not exists player_gm_poss (

    pgp_id int primary key generated by default as identity,
    fk_player_id int references players (player_id) not null,
    fk_game_id int references games (game_id) not null,
    -- possesion related
...
comment on table player_gm_poss is 'each record represents a players possession/offensive stats (dribbling, passing, shooting) in a single league game';

create table if not exists player_gm_def (

    pgd_id int primary key generated by default as identity,
    fk_player_id int references players (player_id) not null,
    fk_game_id int references games (game_id) not null,
    stand_tkl_att int,
    stand_tkl_won int,
...
comment on table player_gm_def is 'each record represents a players defensive stats in a single league game';
